...
?>
<form name="search" method="post" action="logsdbtype5search"  id="searchform ><tr><form name="search" method="post" >
...
if(isset($_GET['name']))
{
	$id=$_GET['name']; 
	require_once("includes/logs.php");
	$result=mysqli_query($con,"SELECT * FROM  logs_money WHERE action LIKE '%" . $id . "%' ORDER BY date DESC LIMIT 0,200");
} else {
	$name=$_POST['query']; 
	require_once("includes/logs.php");
	$result=mysqli_query($con,"SELECT * FROM  logs_money WHERE action LIKE '%" . $name . "%' ORDER BY date DESC LIMIT 0,200");
}
